...
interface CardListReportHistoryScreeningScoringProps {
  data: ScreeningScoring[];
  isLoading: boolean;
}

export default function CardListReportHistoryScreeningScoring({
  data,
  isLoading,
}: CardListReportHistoryScreeningScoringProps) {
  if (isLoading) {
    return (
  ...
  return (
    <div className="space-y-4">
      {data.map((item) => (
        <Link
          key={item.id}
          href={`/dashboard/admin/reports/history/screening-scoring/${item.id}`}
