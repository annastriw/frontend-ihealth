// src/components/molecules/card/CardListReportHistoryScreeningHSMBQ.tsx
import { Card, CardHeader, CardTitle } from "@/components/ui/card";
import { Skeleton } from "@/components/ui/skeleton";
import { FileSearch } from "lucide-react";
import Link from "next/link";

export interface ScreeningHSMBQReportItem {
  id: string;
  created_at: string;
  name: string; // misalnya: "Screening HSMBQ oleh John Doe"
}

interface CardListReportHistoryScreeningHSMBQProps {
  data: ScreeningHSMBQReportItem[];
  isLoading: boolean;
}

export default function CardListReportHistoryScreeningHSMBQ({
  data,
  isLoading,
}: CardListReportHistoryScreeningHSMBQProps) {
  if (isLoading) {
    return (
  ...
  return (
    <div className="space-y-4">
      {data.map((item) => (
        <Link
          key={item.id}
          href={`/dashboard/admin/reports/history/screening-hsmbq/${item.id}`}
