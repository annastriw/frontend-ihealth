...
interface CardListHistoryScreeningDSMQProps {
  data: ScreeningDSMQHistoryItem[];
  isLoading: boolean;
}

export default function CardListHistoryScreeningDSMQ({
  data,
  isLoading,
}: CardListHistoryScreeningDSMQProps) {
  if (isLoading) {
    return (
  ...
  if (!data || data.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center space-y-4 py-10 text-center">
        ...
        <p className="text-muted-foreground">Belum ada riwayat screening DSMQ.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      {data.map((item) => {
        ...
        return (
          <Link
            href={`/dashboard/history/screening-dsmq/${item.id}`}
            key={item.id}
