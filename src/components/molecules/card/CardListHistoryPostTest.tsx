  ...
  return (
    <div className="space-y-4">
      {data.map((postTestHistory) => {
        const formattedDate = format(new Date(postTestHistory.created_at), "dd MMMM yyyy", {
          locale: idLocale,
        });
              ...
              <Card className="border-muted group-hover:bg-muted w-full border-2 shadow-transparent">
                <CardHeader className="space-y-1">
                  ...
                  <p className="text-muted-foreground text-sm">
                    {formattedDate} &middot; Total Skor:{" "}
                    <span className="font-semibold text-green-600">{postTestHistory.sum_score}</span>
                  </p>
                </CardHeader>
              </Card>
            </div>
