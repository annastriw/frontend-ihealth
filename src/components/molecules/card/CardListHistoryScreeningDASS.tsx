// src/components/molecules/card/CardListHistoryScreeningDASS.tsx
"use client";
...
interface CardListHistoryScreeningDASSProps {
  data: ScreeningDASSHistoryItem[];
  isLoading: boolean;
}

export default function CardListHistoryScreeningDASS({
  data,
  isLoading,
}: CardListHistoryScreeningDASSProps) {
  if (isLoading) {
    return (
  ...
  return (
    <div className="space-y-4">
      {data.map((item) => {
        ...
        return (
          <Link
            href={`/dashboard/history/screening-dass/${item.id}`}
            key={item.id}
