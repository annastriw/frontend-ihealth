...
interface CardListHistoryScreeningHSMBQProps {
  data: ScreeningHSMBQHistoryItem[];
  isLoading: boolean;
}

export default function CardListHistoryScreeningHSMBQ({
  data,
  isLoading,
}: CardListHistoryScreeningHSMBQProps) {
  if (isLoading) {
    return (
  ...
  if (!data || data.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center space-y-4 py-10 text-center">
        ...
        <p className="text-muted-foreground">Belum ada riwayat screening HSMBQ.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      {data.map((item) => {
        ...
        return (
          <Link
            href={`/dashboard/history/screening-hsmbq/${item.id}`}
            key={item.id}
