// src/components/molecules/card/CardListReportHistoryScreeningDSMQ.tsx
...
export interface ScreeningDSMQReportItem {
  id: string;
  created_at: string;
  name: string; // misalnya: "Screening DSMQ oleh John Doe"
}

interface CardListReportHistoryScreeningDSMQProps {
  data: ScreeningDSMQReportItem[];
  isLoading: boolean;
}

export default function CardListReportHistoryScreeningDSMQ({
  data,
  isLoading,
}: CardListReportHistoryScreeningDSMQProps) {
  if (isLoading) {
    return (
  ...
  return (
    <div className="space-y-4">
      {data.map((item) => (
        <Link
          key={item.id}
          href={`/dashboard/admin/reports/history/screening-dsmq/${item.id}`}
